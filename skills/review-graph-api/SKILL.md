...
Ask for:
- **Documentation Plan** — generated by the `generate-doc-plan` skill. Saved at `temp-docstubs/{workload-name}/documentation-plan.md`. Ask the user for the workload name or search `temp-docstubs/` for available plans. Enables completeness and accuracy validation.
